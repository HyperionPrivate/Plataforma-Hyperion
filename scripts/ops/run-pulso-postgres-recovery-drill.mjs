...
export function parsePulsoMigrationReceipt(output) {
  ...
  if (JSON.stringify(receipt.skipped) !== JSON.stringify(EXPECTED_MIGRATIONS)) {
    throw new Error(`restored PULSO validation did not skip exactly 001-004: ${receipt.skipped.join(", ")}`);
  }
  return receipt;
}
...
function usage() {
  return `Usage:
  node scripts/ops/run-pulso-postgres-recovery-drill.mjs --confirm "${DRILL_CONFIRMATION}" [--project ${PROJECT_PREFIX}-<id>]

This opt-in drill creates only a fresh, isolated Docker Compose project, performs a real
PostgreSQL backup and restore, verifies the provider ledger, v4 catalog, owner-local SOFIA marker, ownership,
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (entry.isDirectory()) visit(absolute);
        ...
        rows.push(`${path.relative(root, absolute).replaceAll("\\", "/")}\t${sha256(readFileSync(absolute))}`);
      } else {
...
function sourceEvidence(contextDirectory) {
  ...
  const commandRows = commandSources
    .map((file) => `${path.relative(repositoryRoot, file).replaceAll("\\", "/")}\t${sha256(readFileSync(file))}`)
    .sort();
  return {
    branch,
    revision,
    ...
    closure: hashDirectoryClosure(contextDirectory),
    migrationSqlClosure: hashDirectoryClosure(path.join(repositoryRoot, "packages", "pulso-migrations", "sql")),
...
export function runDrill(options) {
  ...
  try {
    ...
    const source = sourceEvidence(path.join(contextOutputRoot, "pulso"));
    ownsProject = true;
    ...
    if (schemaVersion(compose, SOURCE_DATABASE) !== EXPECTED_SCHEMA_VERSION) {
      throw new Error("source PULSO schema is not at provider version 4 / 004");
    }
    if (sofiaSchemaVersion(compose, SOURCE_DATABASE) !== EXPECTED_SOFIA_SCHEMA_VERSION) {
      throw new Error("source SOFIA schema is not at owner-local version 1 / 003");
    }
