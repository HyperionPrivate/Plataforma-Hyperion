...
pulso_database="${PULSO_POSTGRES_DB:-}"
...
[[ "${pulso_database}" =~ ^hyperion_pulso(_[a-z0-9_]+)?$ ]] \
  || fail "PULSO_POSTGRES_DB must explicitly name a hyperion_pulso logical database"
[[ -f "${backup_script}" && ! -L "${backup_script}" ]] || fail "base backup script is unavailable"
for required_command in bash docker env stat uname; do
...
if [[ "${test_mode}" == "1" ]]; then
  ...
  timestamp="${PULSO_BACKUP_TIMESTAMP:-}"
...
[[ "${backup_directory}" == */pulso ]] || fail "PULSO backups must use a dedicated pulso directory"
...
if [[ "${platform}" =~ ^(MINGW|MSYS|CYGWIN) ]]; then
  ...
  [[ "${expected_docker_endpoint}" == npipe://* ]] \
    || fail "PULSO backup requires a local npipe:// Docker endpoint on Windows"
  ...
  [[ "${expected_docker_endpoint}" == unix://* ]] \
    || fail "PULSO backup requires a local unix:// Docker endpoint"
...
exec "${sanitized_environment[@]}" \
  HYPERION_BACKUP_PROFILE=pulso \
  HYPERION_BACKUP_DATABASE="${pulso_database}" \
  HYPERION_BACKUP_PREFIX=pulso \
