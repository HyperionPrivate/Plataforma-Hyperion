...
database="${ACCESS_POSTGRES_DB:-}"

[[ "${test_mode}" == "0" || "${test_mode}" == "1" ]] || fail "invalid test mode"
[[ "${database}" =~ ^hyperion_access(_[a-z0-9_]+)?$ ]] \
  || fail "ACCESS_POSTGRES_DB must explicitly name a hyperion_access logical database"
...
if [[ "${test_mode}" == "1" ]]; then
  ...
  timestamp="${ACCESS_BACKUP_TIMESTAMP:-}"
...
[[ "${backup_directory}" == */access ]] || fail "Access backups must use a dedicated access directory"
...
exec "${sanitized_environment[@]}" \
  HYPERION_BACKUP_PROFILE=access \
  HYPERION_BACKUP_DATABASE="${database}" \
  HYPERION_BACKUP_PREFIX=access \
