...
lumen_database="${LUMEN_POSTGRES_DB:-}"

[[ "${test_mode}" == "0" || "${test_mode}" == "1" ]] || fail "invalid test mode"
[[ "${lumen_database}" =~ ^hyperion_lumen(_[a-z0-9_]+)?$ ]] \
  || fail "LUMEN_POSTGRES_DB must explicitly name a hyperion_lumen logical database"
...
if [[ "${test_mode}" == "1" ]]; then
  ...
  timestamp="${LUMEN_BACKUP_TIMESTAMP:-}"
...
[[ "${backup_directory}" == */lumen ]] || fail "LUMEN backups must use a dedicated lumen directory"
...
exec "${sanitized_environment[@]}" \
  HYPERION_BACKUP_PROFILE=lumen \
  HYPERION_BACKUP_DATABASE="${lumen_database}" \
  HYPERION_BACKUP_PREFIX=lumen \
