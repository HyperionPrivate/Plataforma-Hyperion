...
fail() {
  printf 'Access restore failed: %s\n' "$1" >&2
  exit 1
}
...
test_mode="${ACCESS_OPS_TEST_MODE:-0}"
archive="${ACCESS_RESTORE_ARCHIVE:-}"
target_database="${ACCESS_RESTORE_DATABASE:-}"
expected_sha256="${ACCESS_RESTORE_SHA256:-}"
confirmation="${ACCESS_RESTORE_CONFIRM:-}"

[[ "${test_mode}" == "0" || "${test_mode}" == "1" ]] || fail "invalid test mode"
[[ "${target_database}" =~ ^hyperion_access(_[a-z0-9_]+)?$ ]] \
  || fail "ACCESS_RESTORE_DATABASE must explicitly name a hyperion_access logical database"
[[ "${expected_sha256}" =~ ^[a-f0-9]{64}$ && ! "${expected_sha256}" =~ ^0{64}$ ]] \
  || fail "ACCESS_RESTORE_SHA256 must be an exact non-zero SHA-256"
expected_confirmation="RESTORE ACCESS ${target_database} SHA256 ${expected_sha256}"
[[ "${confirmation}" == "${expected_confirmation}" ]] \
  || fail "ACCESS_RESTORE_CONFIRM must equal '${expected_confirmation}'"
...
if [[ "${test_mode}" == "1" ]]; then
  test_root="${ACCESS_OPS_TEST_ROOT:-}"
  [[ -n "${test_root}" ]] || fail "ACCESS_OPS_TEST_ROOT is required in test mode"
  backup_directory="${ACCESS_BACKUP_DIR:-${test_root}/backups/access}"
  compose_file="${ACCESS_OPS_COMPOSE_FILE:-}"
  environment_file="${ACCESS_OPS_ENV_FILE:-}"
  [[ -n "${compose_file}" && -n "${environment_file}" ]] \
    || fail "test mode requires ACCESS_OPS_COMPOSE_FILE and ACCESS_OPS_ENV_FILE"
else
  test_root=""
...
[[ "${backup_directory}" == */access ]] || fail "Access restores must use the dedicated access backup directory"
[[ -n "${archive}" ]] || fail "ACCESS_RESTORE_ARCHIVE is required"
archive="$(realpath -e -- "${archive}")"
backup_directory="$(realpath -m -- "${backup_directory}")"
[[ "${archive}" == "${backup_directory}/"* ]] || fail "restore archive is outside the Access backup directory"
[[ "$(basename -- "${archive}")" =~ ^access-[0-9]{8}T[0-9]{6}Z\.dump\.gz$ ]] \
  || fail "Access restore archive must use the access-<UTC timestamp>.dump.gz name"
actual_sha256="$(sha256sum -- "${archive}" | awk '{ print $1 }')"
[[ "${actual_sha256}" == "${expected_sha256}" ]] || fail "restore archive SHA-256 does not match ACCESS_RESTORE_SHA256"
...
exec "${sanitized_environment[@]}" \
  HYPERION_RESTORE_PROFILE=access \
  HYPERION_RESTORE_ARCHIVE="${archive}" \
  HYPERION_RESTORE_DATABASE="${target_database}" \
  HYPERION_RESTORE_OWNER=hyperion_access_migrator \
