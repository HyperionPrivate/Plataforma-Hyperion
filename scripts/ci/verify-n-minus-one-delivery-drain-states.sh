...
cleanup_probe() {
  if [[ -n $probe_id ]]; then
    "${compose[@]}" exec -T postgres \
      ...
      -U "${POSTGRES_USER:-hyperion}" -d "${POSTGRES_DB:-hyperion}" --file=- >/dev/null <<'SQL'
delete from channel_runtime.outbox_events where id = :'probe_id'::uuid;
SQL
    probe_id=""
  fi
}
...
insert_probe() {
  ...
  if [[ ! $probe_id =~ ^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$ ]]; then
    probe_id=""
    echo "delivery drain state probe did not allocate a valid synthetic identifier" >&2
    exit 1
  fi

  "${compose[@]}" exec -T postgres \
    psql -X -q -v ON_ERROR_STOP=1 -v probe_id="$probe_id" -v probe_status="$status" \
    -U "${POSTGRES_USER:-hyperion}" -d "${POSTGRES_DB:-hyperion}" --file=- >/dev/null <<'SQL'
insert into channel_runtime.outbox_events (
  ...
  :'probe_id'::uuid, gen_random_uuid(), 'channel.delivery.updated.v1', 1,
