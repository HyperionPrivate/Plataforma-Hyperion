...
Hyperion usa una sola cuenta NATS, `HYPERION`, para que catorce durables administrados compartan el stream
`HYPERION_EVENTS`: once activos, dos temporales de compatibilidad v1 (Channel y PULSO) y uno de drenaje Audit
...
El overlay exige siete passwords diferentes, de 24 caracteres como mínimo, y
...
| ---------- | ------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `access`   | snapshots neutrales de tenant para Channel; snapshots y grants para LUMEN | ninguno                                                    |
| `channel`  | inbound v1/v2, delivery, auditoría y DLQ del snapshot Access              | `channel_access_tenant_snapshot_v1`                        |
...
| `topology` | sólo API administrativa JetStream                                         | provisiona el stream y los catorce durables                |
...
tiempo que la retención del stream. La cuenta reserva hasta 19 consumidores: los catorce administrados y cinco
...
`access.tenant.snapshot.v1` es un contrato neutral propiedad de Access. La identidad `access` sólo publica su
subject; `channel` es la única identidad runtime que puede consultar, extraer y confirmar el durable
`channel_access_tenant_snapshot_v1`, además de publicar `hyperion.dlq.access.tenant.snapshot.v1`. Channel aplica el
inbox y su proyección local de forma atómica, por lo que la redelivery conserva idempotencia sin leer tablas Access.
...
1. provisionar los catorce durables y desplegar primero consumidores capaces de recibir v1 y v2;
