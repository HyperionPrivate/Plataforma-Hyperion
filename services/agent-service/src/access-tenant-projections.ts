...
/**
 * Registers the Access-owned tenant lifecycle feed at SOFIA's internal edge.
...
export function registerAccessTenantProjectionRoutes(
  ...
  credential = readInternalCredential(process.env, "ACCESS_TO_SOFIA_TOKEN"),
...
export async function consumeAccessTenantSnapshot(
  ...
  return db.transaction(async (transaction) => {
    const claim = await transaction.query<{ id: string }>(
      `insert into agent_runtime.access_projection_inbox (
    ...
    await transaction.query("select pg_advisory_xact_lock(hashtextextended($1, 0))", [
      `sofia:access-tenant-snapshot:${event.tenantId}`
    ]);
...
async function readInboxReplay(
  ...
  const existing = await db.query<InboxRow>(
    `select envelope_hash as "envelopeHash", result
       from agent_runtime.access_projection_inbox
  ...
  await db.query(
    `insert into agent_runtime.tenant_snapshots (
     ...
     on conflict (tenant_id) do update set
       ...
       updated_at = now()
     where agent_runtime.tenant_snapshots.tenant_id = excluded.tenant_id`,
    [
      event.tenantId,
...
async function persistInboxResult(
  ...
  await db.query(
    `update agent_runtime.access_projection_inbox
