...
export function resolveAgencyFromGeo(input: {
  ...
  const agency = input.agencia?.trim();
  const fallbackTag = input.fallbackTag?.trim();
  if (agency?.toUpperCase().startsWith("AG_")) {
    return { tag: agency.toUpperCase().replace(/\s+/g, " "), source: "agency_tag" };
  }
  const code = agency?.toUpperCase();
  if (code && /^[A-Z0-9][A-Z0-9_-]{1,39}$/.test(code)) {
    return { code, tag: fallbackTag, source: "agency_code" };
  }
  if (fallbackTag) return { tag: fallbackTag, source: "provider_tag" };
  return { source: input.ciudad || input.agencia ? "unmapped" : "empty" };
}
...
export function normalizeLiwaPayload(raw: Record<string, unknown>): NormalizedLiwaPayload {
  ...
  const agencia =
    String(body.agency_code ?? body.agencia ?? fields.agency_code ?? fields.agencia ?? "").trim() || undefined;
  const providerAgencyTag = String(body.agency_tag ?? fields.agency_tag ?? "").trim() || undefined;
  const geo = resolveAgencyFromGeo({ ciudad, agencia, fallbackTag: providerAgencyTag });
