...
export interface LiwaOutboxDelivery {
  id: string;
...
interface ClaimedLiwaOutboxRow {
  eventId: string;
  tenantId: string | null;
  eventType: string;
  payload: Record<string, unknown>;
...
export interface LiwaOutboxInsert {
  eventId: string;
  eventType: string;
  tenantId: string;
...
export class PostgresLiwaOutbox {
  constructor(
    private readonly db: DatabaseClient,
    private readonly workerId: string
  ) {}

  async claim(limit: number): Promise<LiwaOutboxDelivery[]> {
    const result = await this.db.query<ClaimedLiwaOutboxRow>(
      `with candidates as (
         select event_id
         from liwa.outbox_events
       ...
       update liwa.outbox_events event
...
export async function insertLiwaOutboxEvent(db: DatabaseExecutor, event: LiwaOutboxInsert): Promise<void> {
  await db.query(
    `insert into liwa.outbox_events (
