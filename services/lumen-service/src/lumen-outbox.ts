...
export interface LumenOutboxDelivery {
  id: string;
  tenantId: string;
  type: string;
  version: number;
...
export class PostgresLumenOutbox {
  ...
  async claim(limit: number): Promise<LumenOutboxDelivery[]> {
    const result = await this.db.query<ClaimedOutboxRow>(
      `with terminalized as (
         ...
         select id from lumen.outbox_events
       ...
       update lumen.outbox_events event
