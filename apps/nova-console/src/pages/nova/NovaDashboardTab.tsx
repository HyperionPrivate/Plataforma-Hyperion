...
export function NovaDashboardTab({
  ...
  const funnel = useMemo(() => {
    ...
    return CRM_STAGES.filter((stage) => stage !== "lost").map((stage, index) => {
    ...
    <div className="col" style={{ gap: 16 }}>
      ...
      <Card>
        ...
        <p className="muted tiny" style={{ marginBottom: 8 }}>
          Inicializa el snapshot del tenant y su catálogo operativo autorizado.
        </p>
        <button className="btn btn-primary" type="button" disabled={!canBootstrap} onClick={onBootstrap}>
          Inicializar tenant
        </button>
      </Card>
    </div>
