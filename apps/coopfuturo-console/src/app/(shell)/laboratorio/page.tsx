...
export default function LaboratorioPage() {
  ...
  const [liwaEvent, setLiwaEvent] = useState("document_received");
  ...
  async function onSimulateLiwa() {
    ...
    try {
      const res = await simulateLiwaEvent({
        ...
        phone,
        name,
        ciudad: liwaCiudad || undefined,
        score: liwaEvent === "csat" ? 5 : undefined,
        filename: liwaEvent === "document_received" ? "orden_matricula.pdf" : undefined,
      });
      setLastResult(JSON.stringify(res, null, 2));
      toast.success("Evento LIWA simulado", {
        description: `${liwaEvent} → ${String((res as { accepted?: boolean }).accepted ?? res.ok)}`,
      });
    } catch (err) {
      toast.error("Falló simulación LIWA", {
        description: err instanceof Error ? err.message : "Error",
      });
    } finally {
      setBusy(false);
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <ChartCard title="Simular evento LIWA">
          <div className="space-y-3 p-1">
            <p className="text-xs text-[var(--muted)]">
              Prueba el webhook PULSO sin configurar el builder LIWA. Eventos: documento →
              prequal → handoff (tag AG_* por ciudad).
            </p>
            <label className="block text-sm">
              Evento
              <select
                ...
                value={liwaEvent}
                onChange={(e) => setLiwaEvent(e.target.value)}
              >
                <option value="document_received">document_received</option>
                <option value="prequal_completed">prequal_completed</option>
