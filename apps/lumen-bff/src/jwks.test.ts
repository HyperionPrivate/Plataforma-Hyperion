...
describe("LUMEN BFF local Access token verification", () => {
  ...
  it("enforces signature, issuer and LUMEN audience while tolerating a temporary JWKS outage", async () => {
    ...
    expect((await verifier.resolve(createToken(privateKey, nowMs, "lumen-bff")))?.grants[0]?.productId).toBe("LUMEN");
    expect(await verifier.resolve(createToken(privateKey, nowMs, "pulso-bff"))).toBeUndefined();
    expect(await verifier.resolve(createToken(privateKey, nowMs, "lumen-bff", { iat: undefined }))).toBeUndefined();
    expect(
      await verifier.resolve(createToken(privateKey, nowMs, "lumen-bff", { exp: Math.floor(nowMs / 1000) + 931 }))
    ).toBeUndefined();
    requestFetch.mockRejectedValueOnce(new Error("Access unavailable"));
    nowMs += 2_000;
    expect((await verifier.resolve(createToken(privateKey, nowMs, "lumen-bff")))?.operator.email).toBe(
  ...
  it("coalesces unknown-kid and outage refresh storms behind a cooldown while preserving rotation", async () => {
    ...
    expect(await verifier.resolve(createToken(first.privateKey, nowMs, "lumen-bff"))).toBeDefined();
    const misses = await Promise.all(
      Array.from({ length: 20 }, (_, index) =>
        verifier.resolve(createToken(first.privateKey, nowMs, "lumen-bff", {}, `random-kid-${index}`))
      )
    );
    ...
    expect(
      await verifier.resolve(createToken(rotated.privateKey, nowMs, "lumen-bff", {}, "access-key-2"))
    ).toBeUndefined();
    expect(requestFetch).toHaveBeenCalledTimes(2);
    nowMs += 5_001;
    expect(
    ...
    const stale = await Promise.all(
      Array.from({ length: 12 }, () =>
        verifier.resolve(createToken(rotated.privateKey, nowMs, "lumen-bff", {}, "access-key-2"))
      )
    );
    ...
    expect(
      await verifier.resolve(createToken(first.privateKey, nowMs, "lumen-bff", {}, "unknown-during-outage"))
    ).toBeUndefined();
    expect(requestFetch).toHaveBeenCalledTimes(4);
    nowMs += 5_001;
    expect(
