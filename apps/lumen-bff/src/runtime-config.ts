...
export interface LumenBffServiceOrigins {
  access: string;
  lumen: string;
}
...
export function readLumenBffServiceOrigins(environment: NodeJS.ProcessEnv): LumenBffServiceOrigins {
  return Object.freeze({
    access: readServiceOrigin(environment, "ACCESS_SERVICE_URL", "http://localhost:8081"),
    lumen: readServiceOrigin(environment, "LUMEN_SERVICE_URL", "http://localhost:8090")
  });
}
