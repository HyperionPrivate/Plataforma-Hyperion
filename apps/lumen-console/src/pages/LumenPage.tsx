...
import { clinicalFacilitiesFromWorklist, lumenPath, useLumenContext } from "../lib/context.js";
import {
...
import { hasLumenCapability } from "../lib/rbac.js";
...
export function LumenPage() {
  const { tenant, grant, activeFacilityId, replaceClinicalFacilities } = useLumenContext();
  ...
  const canWrite = hasLumenCapability(grant, "lumen:write");
  ...
  const visibleWorklist = useMemo(
    () => lumenWorklistForFacility(worklist, activeFacilityId),
    [activeFacilityId, worklist]
  );
  ...
  const refreshWorklist = useCallback(
    async (signal?: AbortSignal) => {
      ...
      setWorklist(rows);
      replaceClinicalFacilities(clinicalFacilitiesFromWorklist(rows));
      return rows;
    },
    [replaceClinicalFacilities, tenant.id]
  );
  ...
  }, [discardRecording, replaceClinicalFacilities, tenant.id]);
