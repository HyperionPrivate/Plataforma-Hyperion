...
| NOV-003 | Aplicar compliance gate de ventana horaria, opt-out y frecuencia antes de cada contacto. | `parcial` | El gate único cubre llamada manual/campaña, festivos, weekdays, supresión WA y frecuencias; faltan política legal aprobada y E2E PostgreSQL.                 |
...
| NOV-007 | Entregar handoff por sede y grupo asesor sin grants implícitos.                          | `parcial` | Claims, CRM, contactos y llamadas manuales usan identidad firmada y grants explícitos por sede; falta proyección automática desde Access y concurrencia E2E. |
| NOV-008 | Registrar CSAT y outcomes del core financiero sin acoplar NOVA al ledger externo.        | `parcial` | Live exige HTTPS, token y timeout; contract no es default y está prohibido en producción. Falta acreditar el conector e idempotencia E2E.                    |
...
| NOV-013 | Propagar `contact_id` y `enrollment_id` en voz y eventos post-call.                      | `parcial` | La correlación durable cubre request, poller, webhook y reconciliación; falta evidencia con proveedor real.                                                  |
| NOV-014 | Recuperar outcomes del dialer mediante poller/reconciliación cuando falte el webhook.    | `parcial` | Hay dedupe antes de marcar y escalamiento de outcomes ausentes; falta prueba de pérdida real y recuperación bajo carga.                                      |
...
| NOV-017 | Mover outbox agotado a DLQ por contexto y permitir listado/redrive auditado.             | `parcial` | Hay checks operativos alertables; faltan integrar el monitor, aprobar retención y probar redrive E2E por componente.                                         |
| NOV-018 | Listar/buscar contactos y avanzar el lifecycle de `campaign_enrollments`.                | `parcial` | Orquestador continuo y prueba unitaria de locking existen; faltan enrolamiento UI y concurrencia PostgreSQL E2E.                                             |
...
El alcance congelado de esta remediación está en [VOICE-ORCHESTRATOR-SCOPE.md](nova/VOICE-ORCHESTRATOR-SCOPE.md).
La línea base operativa y sus bloqueos están en [VOICE-OPERATIONS.md](nova/VOICE-OPERATIONS.md).
