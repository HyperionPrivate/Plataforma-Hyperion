...
Los roles de producto y sus capacidades están versionados por PULSO en
[`@hyperion/pulso-contracts`](../../packages/pulso-contracts/src/index.ts). La migración heredada
[007-operator-roles.sql](../../packages/migrations/sql/007-operator-roles.sql) conserva únicamente la representación
transicional de operadores mientras Access completa su separación.
...
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| PUL-001 | Registrar PULSO IRIS como producto y mantener a SOFÍA como su agente/capacidad, con propiedad técnica separada.                                                            | Implementado                                                                                                                                                                                                                                                | [001-pulso-autonomous-baseline.sql](../../packages/pulso-migrations/sql/001-pulso-autonomous-baseline.sql), [data-ownership.json](../architecture/data-ownership.json)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| PUL-003 | Habilitar y configurar el producto por tenant, resolviendo el contexto operativo mediante grants e identificadores UUID sin depender del bootstrap de un cliente concreto. | Parcial/transición: la consola y el BFF vigentes derivan los tenants autorizados de grants PULSO y exigen UUID; el migrador global heredado, todavía ejecutable, conserva el seed CEDCO por slug en `004` y la cadena de prompts específica en `012`–`016`. | [app.ts](../../apps/pulso-bff/src/app.ts), [app.tsx](../../apps/pulso-console/src/app.tsx), [004-cedco-catalog.sql](../../packages/migrations/sql/004-cedco-catalog.sql), [012-whatsapp-sofia-runtime.sql](../../packages/migrations/sql/012-whatsapp-sofia-runtime.sql), [013-sofia-confirmation-protocol.sql](../../packages/migrations/sql/013-sofia-confirmation-protocol.sql), [014-sofia-local-time-protocol.sql](../../packages/migrations/sql/014-sofia-local-time-protocol.sql), [015-sofia-fresh-availability.sql](../../packages/migrations/sql/015-sofia-fresh-availability.sql), [016-sofia-search-constraints.sql](../../packages/migrations/sql/016-sofia-search-constraints.sql), [DEBT-022](../catalogs/debt.v1.json), [inventario congelado](../../scripts/architecture/legacy-global-migrations.json) |
...
| PUL-033 | Cancelar una cita futura mediante una transición explícita, idempotente y auditada.                                                       | Implementado: los reintentos concurrentes equivalentes convergen y los intentos con actor o motivo divergente fallan con conflicto, sin duplicar transición ni auditoría. | [appointment-routes.ts](../../services/pulso-iris-service/src/appointment-routes.ts), [appointment-routes.integration.test.ts](../../services/pulso-iris-service/src/appointment-routes.integration.test.ts), [sofia-tools-routes.ts](../../services/pulso-iris-service/src/sofia-tools-routes.ts) |
...
| PUL-080 | Crear, pausar y consultar campañas con segmento, canales, cadencia y presupuesto lógico.                                                         | Simulado: CRUD y visualización disponibles, sin motor externo de contacto.                         | [operations-routes.ts](../../services/pulso-iris-service/src/operations-routes.ts), [CampaignsPage.tsx](../../apps/pulso-console/src/pages/CampaignsPage.tsx)                    |

### 5.6 Consola, métricas y auditoría

| ID      | Requisito                                                                                                                                 | Estado actual                                                                                                                          | Evidencia versionada                                                                                                                                                                                                                                    |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PUL-090 | Mostrar operación, conversaciones, agenda, configuración, RPA, campañas y BI con permisos por rol.                                        | Parcial: superficies principales disponibles; algunas dependen de datos simulados.                                                     | [pulso-console](../../apps/pulso-console/src), [analytics-routes.ts](../../services/pulso-iris-service/src/analytics-routes.ts)                                                                                                                         |
| PUL-091 | Calcular métricas desde datos del dominio y distinguir valores medidos de líneas base configurables.                                      | Parcial: existen consultas; algunas líneas base siguen codificadas y no representan costos observados.                                 | [analytics-routes.ts](../../services/pulso-iris-service/src/analytics-routes.ts), [BiPage.tsx](../../apps/pulso-console/src/pages/BiPage.tsx)                                                                                                           |
...
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PUL-200 | Cada contexto debe acceder por SQL únicamente a sus tablas; las dependencias nuevas usan HTTP o eventos versionados.                                   | Parcial: el runtime current cerró SOFÍA → PULSO y readiness cruzada; 038 conserva adaptadores v1, FKs históricas y una allow-list SQL temporal sólo para rollback N−1. |
...
| PUL-203 | La identidad de base de datos y los permisos de runtime deben aplicar mínimo privilegio.                                                               | Parcial: el migrador PULSO verifica cinco roles provider-owned y ACL fail-closed; el stack global mantiene grants históricos hasta el cutover.                         |
...
| PUL-209 | Backups, restauración y migraciones deben poder separarse por contexto antes de declarar autonomía completa.                                           | Parcial: existen `hyperion_pulso`, migrador, ledger, roles y Compose propios; faltan cutover y drill de backup/restore del ambiente objetivo.                          |
...
| PUL-211 | SOFÍA debe operar como contexto técnico autónomo con readiness, trabajos, inbox/outbox y acceso a otros dominios solo mediante contratos propietarios. | Implementado para el runtime current: marker local, contratos owner-owned y grants Iris revocados (tip 015); bootstrap PULSO ya no lee markers SOFÍA.                  |
...
La evidencia provider-owned de PUL-203/PUL-209/PUL-211 está en
[`@hyperion/pulso-migrations`](../../packages/pulso-migrations),
[`infra/docker-compose.pulso.yml`](../../infra/docker-compose.pulso.yml),
[`003-sofia-readiness-marker.sql`](../../packages/pulso-migrations/sql/003-sofia-readiness-marker.sql),
[`004-access-channel-tenant-projection.sql`](../../packages/pulso-migrations/sql/004-access-channel-tenant-projection.sql),
los entrypoints de [Agent](../../services/agent-service/src/index.ts) y
[Prompt Flow](../../services/prompt-flow-service/src/index.ts), y las
...
`agent_runtime.schema_version`/`sofia` versión 2, mientras Core, Knowledge, Integration y Channel conservan
temporalmente el marker global `pulso_iris.schema_version` versión 15. La migración 004 añade la proyección local
Access→Channel, la 005 Access→Iris, la 006 Access→SOFIA (`agent_runtime.tenant_snapshots`), la 007
Access→Integration (`integration_runtime.tenant_snapshots`) y la 008 Access→Knowledge
(`knowledge_runtime.tenant_snapshots`) sin retirar todavía las FKs históricas: el corte contract requiere
recibo de paridad y cutover. WhatsApp Channel, Iris, SOFIA, Integration y Knowledge aplican la fase migrate
de lectura en runtime (existencia para lecturas; `active` para mutaciones) sin unir `platform.tenants`.
DEBT-001, DEBT-002, DEBT-003, DEBT-004 y DEBT-005 permanecen `retiring` hasta dropear esas FKs. Identity puede
fan-out HTTP del snapshot a Channel, Iris, Agent, Integration y Knowledge.
...
extracción física ya se hayan ejecutado.

## 7. Contratos, eventos y datos

### 7.1 Contratos HTTP

La entrada pública nueva es el hostname edge neutral hacia `pulso-bff`; el gateway global queda como fachada de
compatibilidad temporal. PULSO IRIS publica:
...
Las definiciones y validaciones están en
[`@hyperion/pulso-contracts`](../../packages/pulso-contracts/src/domain.ts), las rutas de dominio en
[pulso-iris-service](../../services/pulso-iris-service/src), la allowlist pública en
[pulso-bff](../../apps/pulso-bff/src/app.ts) y el routing neutral en
[hostname-edge](../../infra/docker/hostname-edge/default.conf.template). Un consumidor no debe consultar tablas de
PULSO IRIS para reemplazar estos contratos.
...
también queda implementado: la ruta pública serializa cancelaciones concurrentes, devuelve el mismo resultado para
un reintento equivalente y rechaza un actor o motivo divergente sin duplicar historial ni auditoría. PUL-093 sigue
fuera de alcance porque exige auditoría de lecturas sensibles y políticas de retención.
...
| CA-PUL-019 | Una base lógica PULSO vacía ejecuta bootstrap → migración → roles con cinco credenciales runtime distintas y sin recibir migraciones ni secretos NOVA/LUMEN.                                                          |
| CA-PUL-020 | El rol SOFÍA no puede ejecutar DML sobre tablas PULSO; el mismo flujo funciona mediante las rutas internas owner-owned y conserva idempotencia.                                                                       |
| CA-PUL-021 | Crear un tenant en Access no ejecuta SQL ni triggers PULSO; el primer uso autorizado materializa una sola configuración de agenda aun con llamadas concurrentes.                                                      |
...
[agent-service](../../services/agent-service/src). El migrador y su matriz se prueban en
[`packages/pulso-migrations`](../../packages/pulso-migrations/src).
