...
import {
  readSofiaTenantSnapshot,
  requireSofiaTenantAccess,
  type SofiaTenantSnapshotStatus
} from "./access-tenant-projections.js";

const TENANT_ID = "00000000-0000-4000-8000-000000000001";

describe("readSofiaTenantSnapshot", () => {
  it("returns null when the local projection is missing", async () => {
    const db = database(vi.fn(async () => ({ rows: [], rowCount: 0 })));
    await expect(readSofiaTenantSnapshot(db, TENANT_ID)).resolves.toBeNull();
  });
  ...
  it("returns status and source version from tenant_snapshots", async () => {
    ...
    await expect(readSofiaTenantSnapshot(db, TENANT_ID)).resolves.toEqual({
      status: "active",
      sourceVersion: 12
    });
  });
...
describe("requireSofiaTenantAccess", () => {
  it("returns 503 when the database is unavailable", async () => {
    const reply = captureReply();
    await expect(requireSofiaTenantAccess(undefined, TENANT_ID, reply, "req-1", "exists")).resolves.toBeUndefined();
  ...
  it("returns 404 when the snapshot is missing", async () => {
    const reply = captureReply();
    const db = database(vi.fn(async () => ({ rows: [], rowCount: 0 })));
    await expect(requireSofiaTenantAccess(db, TENANT_ID, reply, "req-2", "exists")).resolves.toBeUndefined();
  ...
  it("allows reads for paused and archived tenants", async () => {
    for (const status of ["paused", "archived"] as const) {
      ...
      await expect(requireSofiaTenantAccess(db, TENANT_ID, reply, "req-read", "exists")).resolves.toEqual({
        status,
        sourceVersion: 3
      });
  ...
  it("rejects mutations when the tenant is paused or archived", async () => {
    for (const status of ["paused", "archived"] as const) {
      const reply = captureReply();
      const db = snapshotDb(status);
      await expect(requireSofiaTenantAccess(db, TENANT_ID, reply, "req-mut", "active")).resolves.toBeUndefined();
  ...
  it("allows mutations when the tenant is active", async () => {
    ...
    await expect(requireSofiaTenantAccess(db, TENANT_ID, reply, "req-ok", "active")).resolves.toEqual({
      status: "active",
      sourceVersion: 3
    });
    expect(reply.statusCode).toBeUndefined();
...
function snapshotDb(status: SofiaTenantSnapshotStatus): DatabaseClient {
  return database(
    vi.fn(async () => ({
