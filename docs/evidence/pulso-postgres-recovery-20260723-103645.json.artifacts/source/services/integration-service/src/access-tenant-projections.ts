...
export type IntegrationTenantSnapshotStatus = "active" | "paused" | "archived";

export interface IntegrationTenantSnapshot {
  readonly status: IntegrationTenantSnapshotStatus;
  readonly sourceVersion: number;
}

export type IntegrationTenantAccessMode = "exists" | "active";
...
interface IntegrationTenantSnapshotQueryRow {
...
/**
 * Registers the Access-owned tenant lifecycle feed at Integration's internal edge.
...
export function registerAccessTenantProjectionRoutes(
  ...
  credential = readInternalCredential(process.env, "ACCESS_TO_INTEGRATION_TOKEN"),
...
export async function consumeAccessTenantSnapshot(
  ...
  return db.transaction(async (transaction) => {
    const claim = await transaction.query<{ id: string }>(
      `insert into integration_runtime.access_projection_inbox (
    ...
    await transaction.query("select pg_advisory_xact_lock(hashtextextended($1, 0))", [
      `integration:access-tenant-snapshot:${event.tenantId}`
    ]);
    const current = await transaction.query<TenantSnapshotRow>(
      `select source_version::text as "sourceVersion", payload_hash as "payloadHash"
         from integration_runtime.tenant_snapshots
        where tenant_id = $1
        for update`,
      [event.tenantId]
    );
...
/**
 * Reads the local Access→Pulso projection used for runtime eligibility.
 * Does not join platform.tenants; contract FK retirement remains a separate cut.
 */
export async function readIntegrationTenantSnapshot(
  db: DatabaseExecutor,
  tenantId: string
): Promise<IntegrationTenantSnapshot | null> {
  const result = await db.query<IntegrationTenantSnapshotQueryRow>(
 ...
 */
export async function requireIntegrationTenantAccess(
  db: DatabaseExecutor | undefined,
  tenantId: string,
  reply: AccessGateReply,
  requestId: string,
  mode: IntegrationTenantAccessMode
): Promise<IntegrationTenantSnapshot | undefined> {
  if (!db) {
    void reply.code(503).send(envelope({ error: "DATABASE_URL is required" }, requestId));
    return undefined;
  }

  let snapshot: IntegrationTenantSnapshot | null;
  try {
    snapshot = await readIntegrationTenantSnapshot(db, tenantId);
  } catch {
...
async function readInboxReplay(
  ...
  const existing = await db.query<InboxRow>(
    `select envelope_hash as "envelopeHash", result
       from integration_runtime.access_projection_inbox
...
async function upsertTenantSnapshot(
  ...
  await db.query(
    `insert into integration_runtime.tenant_snapshots (
     ...
     on conflict (tenant_id) do update set
       ...
       updated_at = now()
     where integration_runtime.tenant_snapshots.tenant_id = excluded.tenant_id`,
    [
      event.tenantId,
...
async function persistInboxResult(
  ...
  await db.query(
    `update integration_runtime.access_projection_inbox
