...
export type KnowledgeTenantSnapshotStatus = "active" | "paused" | "archived";

export interface KnowledgeTenantSnapshot {
  readonly status: KnowledgeTenantSnapshotStatus;
  readonly sourceVersion: number;
}

export type KnowledgeTenantAccessMode = "exists" | "active";
...
interface KnowledgeTenantSnapshotQueryRow {
...
/**
 * Registers the Access-owned tenant lifecycle feed at Knowledge's internal edge.
...
export function registerAccessTenantProjectionRoutes(
  ...
  credential = readInternalCredential(process.env, "ACCESS_TO_KNOWLEDGE_TOKEN"),
...
export async function consumeAccessTenantSnapshot(
  ...
  return db.transaction(async (transaction) => {
    const claim = await transaction.query<{ id: string }>(
      `insert into knowledge_runtime.access_projection_inbox (
    ...
    await transaction.query("select pg_advisory_xact_lock(hashtextextended($1, 0))", [
      `knowledge:access-tenant-snapshot:${event.tenantId}`
    ]);
    const current = await transaction.query<TenantSnapshotRow>(
      `select source_version::text as "sourceVersion", payload_hash as "payloadHash"
         from knowledge_runtime.tenant_snapshots
        where tenant_id = $1
        for update`,
      [event.tenantId]
    );
...
/**
 * Reads the local Access→Knowledge projection used for runtime eligibility.
 * Does not join platform.tenants; contract FK retirement remains a separate cut.
 */
export async function readKnowledgeTenantSnapshot(
  db: DatabaseExecutor,
  tenantId: string
): Promise<KnowledgeTenantSnapshot | null> {
  const result = await db.query<KnowledgeTenantSnapshotQueryRow>(
  ...
  if (row.status !== "active" && row.status !== "paused" && row.status !== "archived") {
    throw new Error("Integration tenant snapshot status is invalid");
  }
  const sourceVersion = Number(row.sourceVersion);
  if (!Number.isSafeInteger(sourceVersion) || sourceVersion < 1) {
    throw new Error("Integration tenant snapshot source version is invalid");
  }
  return { status: row.status, sourceVersion };
}

/**
 * Gates tenant-scoped Integration routes on the local projection.
 * - exists: snapshot row required (reads)
 * - active: snapshot must be active (mutations / control plane)
 */
export async function requireKnowledgeTenantAccess(
  db: DatabaseExecutor | undefined,
  tenantId: string,
  reply: AccessGateReply,
  requestId: string,
  mode: KnowledgeTenantAccessMode
): Promise<KnowledgeTenantSnapshot | undefined> {
  if (!db) {
    void reply.code(503).send(envelope({ error: "DATABASE_URL is required" }, requestId));
    return undefined;
  }

  let snapshot: KnowledgeTenantSnapshot | null;
  try {
    snapshot = await readKnowledgeTenantSnapshot(db, tenantId);
  } catch {
  ...
  if (mode === "active" && snapshot.status !== "active") {
    void reply.code(403).send(
      envelope(
        {
          error: "Tenant is not active for knowledge operations",
...
async function readInboxReplay(
  ...
  const existing = await db.query<InboxRow>(
    `select envelope_hash as "envelopeHash", result
       from knowledge_runtime.access_projection_inbox
...
async function upsertTenantSnapshot(
  ...
  await db.query(
    `insert into knowledge_runtime.tenant_snapshots (
     ...
     on conflict (tenant_id) do update set
       ...
       updated_at = now()
     where knowledge_runtime.tenant_snapshots.tenant_id = excluded.tenant_id`,
    [
      event.tenantId,
...
async function persistInboxResult(
  ...
  await db.query(
    `update knowledge_runtime.access_projection_inbox
