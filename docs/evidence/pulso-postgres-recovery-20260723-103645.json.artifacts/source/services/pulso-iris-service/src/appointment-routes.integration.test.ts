...
import { createAuditClient, type AuditEmitter, type EmitAuditEventInput } from "./audit-client.js";
...
const durableAudit = createAuditClient({ logger: { warn: () => undefined } });
...
const coordinatorTwoHeaders = { "x-operator-id": "coordinator-two", "x-operator-role": "coordinator" };
...
describeIntegration("pulso-iris appointment lifecycle", () => {
  ...
  it("converges concurrent public cancellation retries and rejects conflicting retries", async () => {
    const pending = await createHybridAppointment(16, "appointment-cancel-idempotent", advisorOneHeaders);
    expect(pending.statusCode).toBe(201);
    const appointmentId = pending.json().data.id as string;
    const reason = "Solicitud cancelada por el usuario";
    const cancellationEventsBefore = events.filter(
      (event) => event.eventType === "appointment.cancelled" && event.entityId === appointmentId
    ).length;
    ...
    expect(
      events.filter((event) => event.eventType === "appointment.cancelled" && event.entityId === appointmentId)
    ).toHaveLength(cancellationEventsBefore + 1);

    const transitions = await client.query<{ count: number }>(
      `select count(*)::int as count
       from pulso_iris.appointment_status_history
       where tenant_id = $1 and appointment_id = $2 and to_status = 'cancelled'`,
      [tenantId, appointmentId]
    );
    expect(transitions.rows[0]?.count).toBe(1);
    const auditOutbox = await client.query<{ count: number }>(
      `select count(*)::int as count
       from pulso_iris.outbox_events
       where tenant_id = $1
         and event_type = 'pulso.audit.event.record.v1'
         and payload->>'eventType' = 'appointment.cancelled'
         and payload->>'entityId' = $2`,
      [tenantId, appointmentId]
    );
    expect(auditOutbox.rows[0]?.count).toBe(1);

    const differentReason = await cancelAppointment(appointmentId, "Motivo de cancelacion diferente");
    expect(differentReason.statusCode).toBe(409);
    expect(differentReason.json().data.code).toBe("idempotency_conflict");

    const differentOperator = await cancelAppointment(appointmentId, reason, coordinatorTwoHeaders);
    expect(differentOperator.statusCode).toBe(409);
    expect(differentOperator.json().data.code).toBe("idempotency_conflict");
    expect(
      events.filter((event) => event.eventType === "appointment.cancelled" && event.entityId === appointmentId)
    ).toHaveLength(cancellationEventsBefore + 1);
  });
