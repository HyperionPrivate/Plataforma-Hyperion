...
export type IrisTenantSnapshotStatus = "active" | "paused" | "archived";

export interface IrisTenantSnapshot {
  readonly status: IrisTenantSnapshotStatus;
  readonly sourceVersion: number;
}

export type IrisTenantAccessMode = "exists" | "active";
...
interface IrisTenantSnapshotQueryRow {
...
/**
 * Registers the Access-owned tenant lifecycle feed at Pulso Iris's internal edge.
...
export function registerAccessTenantProjectionRoutes(
  ...
  credential = readInternalCredential(process.env, "ACCESS_TO_PULSO_TOKEN"),
...
export async function consumeAccessTenantSnapshot(
  ...
  return db.transaction(async (transaction) => {
    const claim = await transaction.query<{ id: string }>(
      `insert into pulso_iris.access_projection_inbox (
    ...
    await transaction.query("select pg_advisory_xact_lock(hashtextextended($1, 0))", [
      `pulso:access-tenant-snapshot:${event.tenantId}`
    ]);
    const current = await transaction.query<TenantSnapshotRow>(
      `select source_version::text as "sourceVersion", payload_hash as "payloadHash"
         from pulso_iris.tenant_snapshots
        where tenant_id = $1
        for update`,
      [event.tenantId]
    );
 ...
 */
export async function readIrisTenantSnapshot(
  db: DatabaseExecutor,
  tenantId: string
): Promise<IrisTenantSnapshot | null> {
  const result = await db.query<IrisTenantSnapshotQueryRow>(
...
export async function requireIrisTenantAccess(
  ...
  if (mode === "active" && snapshot.status !== "active") {
    void reply.code(403).send(
      envelope(
        {
          error: "Tenant is not active for pulso operations",
...
async function readInboxReplay(
  ...
  const existing = await db.query<InboxRow>(
    `select envelope_hash as "envelopeHash", result
       from pulso_iris.access_projection_inbox
  ...
  await db.query(
    `insert into pulso_iris.tenant_snapshots (
     ...
     on conflict (tenant_id) do update set
       ...
       updated_at = now()
     where pulso_iris.tenant_snapshots.tenant_id = excluded.tenant_id`,
    [
      event.tenantId,
...
async function persistInboxResult(
  ...
  await db.query(
    `update pulso_iris.access_projection_inbox
