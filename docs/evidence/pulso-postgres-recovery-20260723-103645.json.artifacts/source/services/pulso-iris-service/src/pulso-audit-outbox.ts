...
export interface PulsoAuditOutboxDelivery {
  id: string;
  tenantId: string;
  type: string;
  version: number;
...
export class PostgresPulsoAuditOutbox {
  ...
  async claim(limit: number): Promise<PulsoAuditOutboxDelivery[]> {
    const result = await this.db.query<ClaimedOutboxRow>(
      `with terminalized as (
         ...
         select id from pulso_iris.outbox_events
      ...
      [this.workerId, Math.max(1, Math.min(20, Math.trunc(limit))), PULSO_AUDIT_EVENT_TYPE]
    );
  ...
  async complete(eventId: string): Promise<void> {
    await this.db.query(
      ...
      [eventId, this.workerId, PULSO_AUDIT_EVENT_TYPE]
    );
  }
  ...
  async fail(eventId: string, errorCode: string): Promise<void> {
    await this.db.query(
      ...
      [eventId, this.workerId, sanitizeErrorCode(errorCode), PULSO_AUDIT_EVENT_TYPE]
    );
  }
}
