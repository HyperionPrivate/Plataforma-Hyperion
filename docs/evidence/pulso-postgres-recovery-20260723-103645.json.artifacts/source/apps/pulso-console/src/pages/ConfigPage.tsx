...
type Tab = "agenda" | "sites" | "integrations";
...
export function ConfigPage() {
  const { session, tenant, grant } = useConsole();
  const [tab, setTab] = useState<Tab>("agenda");
  const visibleTabs = TABS.filter((item) => {
    if (item.id === "integrations") return canViewWhatsAppIntegration(session.operator.role);
    return true;
  });
  const canWriteConfig = can(session.operator.role, "write:config") && hasPulsoCapability(grant, "pulso:write");
