...
import { runLumenMigrationsWithClient, type LumenMigrationClient } from "./runner.js";
...
vi.mock("./schema-manifest.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    assertLumenMigratorDatabaseSecurity: vi.fn(async () => {
      hooks.timeline.push("migrator-security-preflight");
      return {};
    }),
    inspectLumenSchema: vi.fn(async () => {
...
describe("LUMEN migration runner timeout wiring", () => {
  it("configures bounded session timeouts before role preflight and bounded advisory-lock acquisition", async () => {
    ...
    await expect(runLumenMigrationsWithClient(client, sqlDirectory)).rejects.toBe(hooks.inspectionError);
  ...
  it("restores the 300-second budget and never inspects or unlocks when advisory-lock acquisition fails", async () => {
    ...
    await expect(runLumenMigrationsWithClient(client, sqlDirectory)).rejects.toBe(lockError);
...
class TimelineClient implements LumenMigrationClient {
