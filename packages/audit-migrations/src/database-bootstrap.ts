...
const { Client } = pg;
const DATABASE_BOOTSTRAP_LOCK = "audit:logical-database-bootstrap";

export interface AuditDatabaseBootstrapClient {
  query<T = Record<string, unknown>>(sql: string, values?: unknown[]): Promise<{ rows: T[] }>;
}

export async function bootstrapAuditLogicalDatabase(
  adminUrl: string,
  databaseName: string,
  ...
  try {
    await applyAuditLogicalDatabase(client, databaseName, migratorPassword);
  } finally {
    await client.end();
  }
}

export async function applyAuditLogicalDatabase(
  client: AuditDatabaseBootstrapClient,
  databaseName: string,
  migratorPassword: string
): Promise<void> {
  ...
  try {
    ...
    if (database.rows.length > 0 && database.rows[0]?.owner !== AUDIT_MIGRATOR_ROLE) {
      throw new Error(`Audit database ${databaseName} must be owned by ${AUDIT_MIGRATOR_ROLE}`);
    }

    const existingRoles = await inspectPreexistingRoles(client, databaseName);
    await ensureRole(client, AUDIT_MIGRATOR_ROLE, existingRoles.has(AUDIT_MIGRATOR_ROLE), true, migratorPassword);
    await ensureRole(client, AUDIT_RUNTIME_ROLE, existingRoles.has(AUDIT_RUNTIME_ROLE), false);
    ...
    await executeFormatted(
      ...
      [databaseName, AUDIT_MIGRATOR_ROLE]
    );
  } finally {
...
async function inspectPreexistingRoles(
  ...
  for (const role of roles.rows) {
    if (role.unsafe_capabilities || role.has_memberships || role.owns_out_of_scope_objects) {
      throw new Error(`Audit database bootstrap refused pre-existing authority drift for ${role.rolname}`);
    }
  }
...
async function executeFormatted(client: AuditDatabaseBootstrapClient, sql: string, values: unknown[]): Promise<void> {
  const formatted = await client.query<{ statement: string }>(sql, values);
  const statement = formatted.rows[0]?.statement;
  if (!statement) throw new Error("Could not prepare Audit database bootstrap statement");
