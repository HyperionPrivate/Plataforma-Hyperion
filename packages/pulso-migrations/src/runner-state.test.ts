...
describe("PULSO migration state recovery", () => {
  ...
  it("atomically adopts an exact legacy closure and applies the remaining append-only migrations", async () => {
    ...
    hooks.inspections.push(
      ...
      inspection("legacy", undefined, undefined, []),
      inspection("managed", 6, sofiaProjectionName, ledger(checksums))
    );
    const client = new RecordingClient();

    await expect(runPulsoMigrationsWithClient(client, sqlDirectory, emptyManifests())).resolves.toEqual({
      applied: [rolesName, sofiaMarkerName, channelProjectionName, irisProjectionName, sofiaProjectionName],
  ...
  it("accepts a structurally valid managed 002 database and applies 003 through 006", async () => {
    const checksums = await migrationChecksums();
    hooks.inspections.push(
    ...
    await expect(runPulsoMigrationsWithClient(client, sqlDirectory, emptyManifests())).resolves.toEqual({
      applied: [sofiaMarkerName, channelProjectionName, irisProjectionName, sofiaProjectionName],
      adopted: [],
  ...
  it("upgrades an exact managed 003 database with Channel, Iris and SOFIA projections", async () => {
    const checksums = await migrationChecksums();
    hooks.inspections.push(
    ...
    await expect(runPulsoMigrationsWithClient(client, sqlDirectory, emptyManifests())).resolves.toEqual({
      applied: [channelProjectionName, irisProjectionName, sofiaProjectionName],
      adopted: [],
...
async function migrationChecksums(): Promise<Map<string, string>> {
  return new Map(
    await Promise.all(
      [baselineName, rolesName, sofiaMarkerName, channelProjectionName, irisProjectionName, sofiaProjectionName].map(
        async (name) =>
          [
            name,
...
function emptyManifests() {
  ...
  return { legacy: empty, managed: empty, managedByVersion: { 2: empty, 3: empty, 4: empty, 5: empty, 6: empty } };
}
