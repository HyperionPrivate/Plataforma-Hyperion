...
export const PULSO_MIGRATION_TIMEOUTS = {
  lockTimeout: "10s",
  statementTimeout: "300s",
  idleInTransactionSessionTimeout: "60s"
} as const;

export interface PulsoSqlPolicyClient {
 ...
 */
export function assertPulsoProviderSqlPreservesTimeouts(name: string, sql: string): void {
  ...
  if (forbidden !== undefined) {
    throw new Error(`PULSO migration ${name} must not SET, RESET, or disable runner-managed database timeouts`);
  }
}

/** Configure transaction-local limits immediately before provider SQL runs. */
export async function configurePulsoMigrationTimeouts(client: PulsoSqlPolicyClient): Promise<void> {
  await client.query(
    `select
    ...
    [
      PULSO_MIGRATION_TIMEOUTS.lockTimeout,
      PULSO_MIGRATION_TIMEOUTS.statementTimeout,
      PULSO_MIGRATION_TIMEOUTS.idleInTransactionSessionTimeout
    ]
  );
}

/** Bound catalog preflight and every statement outside a migration transaction. */
export async function configurePulsoMigrationSessionTimeouts(client: PulsoSqlPolicyClient): Promise<void> {
  await client.query(
    `select
 ...
 */
export async function acquirePulsoMigrationLock(client: PulsoSqlPolicyClient, key: string): Promise<void> {
  await client.query("select set_config('statement_timeout', $1, false)", [PULSO_MIGRATION_TIMEOUTS.lockTimeout]);
  try {
