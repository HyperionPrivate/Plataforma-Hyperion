...
export interface HttpOutboxDispatcherOptions<TPayload = JsonValue> {
  ...
  /**
   * Adds event-scoped headers (for example a signed tenant/product assertion).
   * Transport-owned headers are written afterwards and cannot be overridden.
   */
  readonly requestHeaders?: (event: ClaimedOutboxEvent<TPayload>) => HeadersInit;
...
export class HttpOutboxDispatcher<TPayload = JsonValue> {
  ...
  readonly #internalToken: string;
  readonly #fetch: HttpOutboxFetch;
  readonly #requestHeaders: NonNullable<HttpOutboxDispatcherOptions<TPayload>["requestHeaders"]>;
  ...
  async #request(event: ClaimedOutboxEvent<TPayload>, body: string): Promise<RequestOutcome | TimeoutOutcome> {
    ...
    const headers = new Headers(this.#requestHeaders(event));
    headers.set("authorization", `Bearer ${this.#internalToken}`);
    headers.set("content-type", "application/json");
    headers.set("x-hyperion-event-id", event.id);
    headers.set("x-hyperion-event-type", event.type);
    headers.set("x-hyperion-event-version", String(event.version));
    headers.set("x-hyperion-worker-id", this.workerId);
